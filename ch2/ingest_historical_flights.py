...
class BeamTransformRecords(beam.DoFn):
    ...
    def process(self, element: dict, *args, **kwargs):
        ...
        try:
            ...
            time_cols = ['departure_time', 'actual_departure_time', 'wheels_off', 'arrival_time', 'actual_arrival_time']
...
def run():
    ...
    with beam.Pipeline(options=options) as p:
        ...
        json_output = (rows
        | beam.Map(lambda e: {k: v if k != 'flight_date' else v.strftime('%Y-%m-%d') for k, v in e.items()})
